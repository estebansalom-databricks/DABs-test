...
AS SELECT * FROM cloud_files("/Workspace/ts_demos/iot_data/turbine", "json", map("cloudFiles.inferColumnTypes" , "true"))
-- COMMAND ----------
...
AS SELECT * FROM cloud_files("/Workspace/ts_demos/iot_data/incoming_data", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Workspace/ts_demos/iot_data/historical_turbine_status", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
-- Builds our tabless used by ML Engineers: join sensor aggregates with wind turbine metadata and historical status
